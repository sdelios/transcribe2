...
if ($idTransActual > 0) {
  ...
  if (!$mysqli->connect_error) {
    ...
    $stmt = $mysqli->prepare('SELECT tActaHtml FROM transcripciones WHERE iIdTrans = ?');
    $stmt->bind_param('i', $idTransActual);
    $stmt->execute();
    $stmt->bind_result($actaHtml);
    if ($stmt->fetch() && !empty($actaHtml)) {
      $tieneActa = true;
      $actaGuardada = $actaHtml;
      // Opcional: también ponerlo en sesión para reusar flujo existente
      $_SESSION['resultado_html'] = $actaGuardada;
    }
    $stmt->close();
    $mysqli->close();
  }
...
<?php else: ?>
  <div class="notice success">Esta transcripción ya tiene un Acta generada.</div>
<?php endif; ?>
...
<section id="resultado" class="card result" style="margin-top:18px;">
  <div class="inner">
    ...
    <div>
        <?php
          if ($hayResultado) {
            // HTML completo devuelto por OpenAI
            $doc = $_SESSION['resultado_html'];

            // 1) Mostrar en iframe (no hereda estilos del sitio)
            ...
            // 2) Botón "Guardar Acta" (si tenemos ID)
            if ($idTransActual) {
              // Guardamos el HTML en un <textarea> oculto para enviarlo al controlador
              $htmlForPost = htmlspecialchars($doc, ENT_NOQUOTES | ENT_SUBSTITUTE, 'UTF-8');
              echo '
              <form id="formGuardarActa" action="index.php?ruta=acta/guardar" method="POST" style="margin-top:12px;">
                <input type="hidden" name="iIdTrans" value="'. (int)$idTransActual .'">
                <textarea name="tActaHtml" id="tActaHtml" style="display:none;">'.$htmlForPost.'</textarea>
                <button type="submit" class="btn2">Guardar Acta</button>
                <span class="muted" style="margin-left:8px;">Se guardará el HTML completo con sus estilos.</span>
              </form>';
            } else {
              echo '<div class="muted">No hay ID de transcripción en esta sesión; no puedo guardar el acta.</div>';
            }

            // Limpia para no re-renderizar en reload
            unset($_SESSION['resultado_html']);
          }
          // Limpia “últimos” pegados
