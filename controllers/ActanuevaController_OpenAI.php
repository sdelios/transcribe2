...
//--------------------------------------------------------------------------------------------
    public function generarSintesis()
    {
        ...
        $r = $chars <= 130000
            ? $this->sintesisSinglePassOpenAI($textoActa, $apiKey, $modelo)
            : $this->sintesisMapReduceOpenAI($textoActa, $apiKey, $modelo);
    ...
    private function sistemaPromptSintesisOpenAI(): string
    {
        return
    ...
    private function sintesisSinglePassOpenAI(string $texto, string $apiKey, string $modelo): array
    {
        $payload = [
            "model"       => $modelo,
            "max_tokens"  => 8000,
            ...
            "messages" => [
                [
                    "role"    => "system",
                    "content" => $this->sistemaPromptSintesisOpenAI()
                ],
                [
                    "role"    => "user",
                    "content" =>
    ...
    private function sintesisMapReduceOpenAI(string $texto, string $apiKey, string $modelo): array
    {
        ...
        foreach ($chunks as $i => $chunk) {
            if ($i > 0) sleep(3);
            $r = $this->extraerEventosChunkOpenAI($chunk, $i + 1, count($chunks), $apiKey, $modelo);
        ...
        return $this->narrativaDesdEventosOpenAI($eventos, $apiKey, $modelo);
    }

    private function extraerEventosChunkOpenAI(string $chunk, int $n, int $total, string $apiKey, string $modelo): array
    {
        $payload = [
            "model"       => $modelo,
            "max_tokens"  => 4000,
    ...
    private function narrativaDesdEventosOpenAI(array $eventos, string $apiKey, string $modelo): array
    {
